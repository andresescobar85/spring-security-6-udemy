...
public class JWTService {
	...
	public<T> T getClaimsFromToken(String token,Function <Claims,T> claimsResolver) {
		...
		final var claims = this.getAllClaimsFromTokens(token);
	...
	public boolean validateToken(String token,UserDetails userDetails) {
		final var usernameFromDetails = userDetails.getUsername();
		final var usernameFromJWT = this.getUserNameFromToken(token);
		
		return (usernameFromDetails.equals(usernameFromJWT)) && !this.isTokenExpired(token);
